 ...
 * @author misa
 */
@Named(value = "modificationEvtMB")
@ViewScoped
public class ModificationEvtMB implements Serializable {

    public ModificationEvtMB() {
    ...
    public void loadEvt() {
        try {
            EvenementDossier d = new EvenementDossier();
            d.setId(idEvt);
            evtDossier = (EvenementDossier) generiqueBean.getService().findById(d);
            idObservateurSelected = evtDossier.getDemandeur();
            idIntervenantSelected = evtDossier.getIdIntervenant();
            TarifsNS t = new TarifsNS();
            t.setId(evtDossier.getIdEvtTarif());
            tarifNSSelected = (TarifsNS) generiqueBean.getService().findById(t);
        } catch (Exception ex) {
            ex.printStackTrace();
            MessageUtil.addFlashErrorMessage("Erreur de chargement");
        }
...
//            tarifNSSelected.setDuree(util.toTime(util.dateToTimeString(dureeTime)));
    ...
    public String valider() {
        try {
            ...
            generiqueBean.getService().update(evtDossier);
            MessageUtil.addFlashInfoMessage("Tâche modifiée");
